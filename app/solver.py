...
from app.paths import PROMPT_SOLVER_V1
...
def build_solver_prompt(extraction: ExtractionResult, analysis: AnalysisResult,
                        ...
                        prompt_path: Path = PROMPT_SOLVER_V1) -> str:
    head = Path(prompt_path).read_text(encoding="utf-8")
    return "\n\n".join([head, pairs_block(extraction, analysis, question_ids)])
        ...
        for st in a.strategies:
            ...
            if not solution.reached_answer:
                report("STRATEGY_DOES_NOT_SOLVE", "high", a.source_question_id,
                       f"{a.source_question_id}: strategy {st.strategy_name!r} did not reach an "
                       f"answer: {solution.notes or 'no reason given'}.")
                continue
...
class Solver:
    ...
    def solve(self, extraction: ExtractionResult, analysis: AnalysisResult) -> list[StrategySolution]:
        ...
        for start in range(0, len(ids), SOLVER_BATCH):
            ...
            prompt = build_solver_prompt(extraction, analysis, batch)
            payload: SolverPayload = self.gemini.generate_json(prompt, SolverPayload)
